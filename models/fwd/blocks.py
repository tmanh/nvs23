...
from .normalization import LinearNoiseLayer


def spectral_conv_function(in_c, out_c, k, p, s):
    return nn.utils.spectral_norm(nn.Conv2d(in_c, out_c, kernel_size=k, padding=p, stride=s))
...
def get_conv_layer(opt):
    return spectral_conv_function if "spectral" in opt.norm_G else conv_function
...
class ResNet_Block(nn.Module):
    def __init__(self, in_c, in_o, opt, downsample=None, norm="batch"):
        ...
        if downsample == "Down" or downsample != "Up" and downsample:
        ...
        self.ch_a = nn.Sequential(norm_1, nn.ReLU(inplace=True), conv_aa, norm_2, nn.ReLU(inplace=True), conv_ab, norm_downsample)
