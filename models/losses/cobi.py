# https://github.com/S-aiueo32/contextual_loss_pytorch/blob/master/tests/test_contextual_bilateral.py
...
class ContextualBilateralLoss(nn.Module):
    ...
    def __init__(
        ...
        assert band_width > 0, 'band_width parameter must be positive.'
        assert loss_type in F.LOSS_TYPES,\
            f'select a loss type from {F.LOSS_TYPES}.'

        self.band_width = band_width

        if use_vgg:
            self.vgg_model = VGG19().to(device)
            self.vgg_layer = vgg_layer
            self.register_buffer(
                name='vgg_mean',
                tensor=torch.tensor(
