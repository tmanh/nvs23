...
class _SynchronizedBatchNorm(_BatchNorm):
    ...
    def forward(self, _input):
        ...
        input_shape = _input.size()
        _input = _input.view(_input.size(0), self.num_features, -1)

        # Compute the sum and square-sum.
        sum_size = _input.size(0) * _input.size(2)
        input_sum = _sum_ft(_input)
        input_ssum = _sum_ft(_input ** 2)
        ...
        if self.affine:
            ...
            output = (_input - _unsqueeze_ft(mean)) * _unsqueeze_ft(
                inv_std * self.weight
            ) + _unsqueeze_ft(self.bias)
        else:
            output = (_input - _unsqueeze_ft(mean)) * _unsqueeze_ft(inv_std)
