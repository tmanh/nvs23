...
def homo_warping(src_fea, src_proj, ref_proj, depth_values):
    ...
    warped_src_fea = F.grid_sample(src_fea, grid.view(batch, num_depth * height, width, 2), mode='bilinear', align_corners=False,
                                   padding_mode='zeros')
...
class Unet_Cost_Volume(nn.Module):
    ...
    def __init__(self, num_filters=32, channels_in=3, channels_out=3, opt=None, explicit_depth=False):
    ...
    def forward(self, input_img, proj_matrices, depth_values):
        ...
        depths_values = depth_values.unsqueeze(1).unsqueeze(2).unsqueeze(-1).unsqueeze(-1)

        e1 = self.conv1(input_img)
        e2 = self.conv2(e1)
        e3 = self.conv3(e2)
        e4 = self.conv4(e3)
        feature = self.feature(e4)
        permuted_feature = feature.view(B, nv, num_filters * 2, H // 16, W // 16)
        ref_volume = feature.view(B, nv, num_filters * 2, H // 16, W // 16).unsqueeze(3).repeat(1, 1, 1, num_depth, 1, 1)
        ...
        for ref_index in range(nv):
            for src_index in range(nv):
                ...
                volume_sq_sum[ref_index] = volume_sq_sum[ref_index] + warped_volume**2
        volume_sum = torch.stack(volume_sum, dim=1)
        volume_sq_sum = torch.stack(volume_sq_sum, dim=1)
        volume_variance = volume_sq_sum.div_(nv).sub_(volume_sum.div_(nv).pow_(2))
        cost_reg = self.CostReg(volume_variance.view(-1, *volume_variance.shape[2:]))
        prob_volume = F.softmax(self.CostProb(cost_reg), dim=2)
        num_inputs = nv
        depths_values = depths_values.repeat(1, num_inputs, 1, 1, 1, 1)
        depths_values = depths_values.view(-1, *depths_values.shape[2:])
        ...
        d4_ = self.conv5(self.up(combine_feature))
        d4 = torch.cat((d4_, e3), dim=1)
        d3_ = self.conv6(self.up(d4))
        d3 = torch.cat((d3_, e2), dim=1)
        d2_ = self.conv7(self.up(d3))
        d2 = torch.cat((d2_, e1), dim=1)
        return self.conv8(self.up(d2))
