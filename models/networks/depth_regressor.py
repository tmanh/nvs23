...
class Depth_Regressor(nn.Module):
    def __init__(self, opt):
        ...
        channels_in = 4 if opt.depth_com else 3
        channels_out = 1
    ...
    def forward(self, depth_input, input_RTs=None, K=None):
        ...
        if H % self.pad == 0 and W % self.pad == 0:
            ...
            regressed_depth = self.complete_depth_with_padding(H, W, depth_input)
        ...
        if self.opt.inverse_depth:
            ...
            regressed_pts = 1.0 / torch.clamp(regressed_pts, min=0.001)
        else:
    ...
    def complete_depth_with_padding(self, H, W, depth_input):
        ...
        return self.model(depth_input)[:, :, 0:H, 0:W]
