...
from models.networks.architectures import ResNetDecoder, UNetDecoder64
...
class Decoder(nn.Module):
    ...
    def forward(self, gen_fs, scale_factor=2.0):
        if self.decode_in_dim != 64:
            gen_fs = self.shallow(gen_fs)

        _, _, H, W = gen_fs.shape
        return self.model(gen_fs, scale_factor=1.0)
