...
class BaseConvDropout(nn.Module):
    def __init__(self, input_channels, output_channels, conv_op=nn.Conv2d, conv_kwargs=None,
        ...
        super(BaseConvDropout, self).__init__()
...
class ConvDropoutNormNonlin(BaseConvDropout):
    def forward(self, x):
        x = self.conv(x)
...
class ConvDropoutNonlinNorm(BaseConvDropout):
    def forward(self, x):
        x = self.conv(x)
        if self.dropout is not None:
            x = self.dropout(x)
