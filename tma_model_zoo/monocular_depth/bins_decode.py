...
class BinsFormerDecodeHead(DepthBaseDecodeHead):
    ...
    def __init__(self, binsformer=True, align_corners=True, norm_cfg=dict(type='BN', requires_grad=True), act_cfg=dict(type='LeakyReLU', inplace=True), dms_decoder=True,
        ...
        for trans_channel in trans_channels:
            self.skip_proj.append(ConvModule(trans_channel, self.conv_dim, kernel_size=1, norm_cfg=norm_cfg, act_cfg=act_cfg))
        ...
        for in_channel in self.in_channels[:self.num_fpn_levels]:
            lateral_conv = ConvModule(in_channel, conv_dim, kernel_size=1, norm_cfg=norm_cfg)
            output_conv = ConvModule(conv_dim, conv_dim, kernel_size=3, stride=1, padding=1, norm_cfg=norm_cfg, act_cfg=act_cfg)
        ...
        if binsformer is False:
            self.pred_depth = ConvModule(self.n_bins, 1, kernel_size=3, stride=1, padding=1)
    ...
    def forward(self, inputs):
        ...
        for idx, feat in enumerate(mlvl_feats):
            ...
            mlvl_masks.append(F.interpolate(img_masks[None], size=feat.shape[-2:]).to(torch.bool).squeeze(0))
            pos.append(self.positional_encoding(mlvl_masks[-1]).flatten(2) + self.level_embed.weight[idx][None, :, None])
        ...
        for item_bin, pred_logit, pred_class in zip(predictions_bins, predictions_logits, predictions_class):  
            ...
            for index, weight in zip(train_cfg["aux_index"], train_cfg["aux_weight"]):
                ...
                if self.binsformer is False:
                    ...
                    if self.classify:
                        ...
                        aux_weight_dict[f"aux_loss_ce_{index}"] = loss_ce

                    if self.with_loss_chamfer:
                        bin = pred_bins[index]
                        bins_loss = self.loss_chamfer(bin, depth_gt) * weight
                        aux_weight_dict[f"aux_loss_chamfer_{index}"] = bins_loss

                aux_weight_dict[f"aux_loss_depth_{index}"] = depth_loss
        ...
        if self.binsformer is False:
            ...
            if self.classify:
                ...
                for index, topk in enumerate(acc):
                    losses[f"ce_acc_level_{index}"] = topk
