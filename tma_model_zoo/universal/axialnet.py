...
class ResAxialAttentionUNet(nn.Module):
    ...
    def _make_layer(self, block, planes, blocks, kernel_size=56, stride=1, dilate=False):
        ...
        layers = [block(self.inplanes, planes, stride, downsample, groups=self.groups, base_width=self.base_width, dilation=previous_dilation, norm_layer=norm_layer, kernel_size=kernel_size)]

        self.inplanes = planes * block.expansion
        if stride != 1:
            kernel_size = kernel_size // 2

        layers.extend(block(self.inplanes, planes, groups=self.groups, base_width=self.base_width, dilation=self.dilation, norm_layer=norm_layer, kernel_size=kernel_size) for _ in range(1, blocks))
...
class medt_net(nn.Module):
    ...
    def _make_layer(self, block, planes, blocks, kernel_size=56, stride=1, dilate=False):
        ...
        layers = [block(
            self.inplanes, planes, stride, downsample, groups=self.groups, base_width=self.base_width, dilation=previous_dilation,
