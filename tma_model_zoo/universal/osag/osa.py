...
from tma_model_zoo.universal.osag.norm import LayerNorm2d
...
class OSABlock(nn.Module):
    ...
    def forward(self, x):
        ...
        return self.layer(x)[:, :, :H, :W].contiguous()
    

class MOSABlock(nn.Module):
    def __init__(self, c_dim=64, d_dim=32, heads=8, window_size=8, n_views=3, dropout=0.0):
        super(MOSABlock, self).__init__()

        self.window_size = window_size
        self.n_views = n_views

        self.cross_view = CrossViewAttention(c_dim, d_dim, heads)

        self.attention = nn.Sequential(
            Conv_PreNormResidual(c_dim, Channel_Attention(dim = c_dim, heads=4, dropout = dropout, window_size = window_size)),
            Conv_PreNormResidual(c_dim, Gated_Conv_FeedForward(dim = c_dim, dropout = dropout))
        )

    def forward(self, cs, ds):
        H, W = cs.shape[-2:]

        x = self.cross_view(cs, ds)

        pad_r = (self.window_size - W % self.window_size) % self.window_size
        pad_b = (self.window_size - H % self.window_size) % self.window_size
        x = F.pad(x, (0, pad_r, 0, pad_b))
        return self.attention(x)[:, :, :H, :W].contiguous()
...
class CrossViewAttention(nn.Module):
    def __init__(
        ...
        self.cqkv = nn.Conv2d(cdim, cdim*3, kernel_size=1, bias=bias)
        self.cqkv_dwconv = nn.Conv2d(cdim*3, cdim*3, kernel_size=3, stride=1, padding=1, groups=cdim*3, bias=bias)

        self.dqk = nn.Conv2d(ddim, ddim*2, kernel_size=1, bias=bias)

        self.project_out = nn.Conv2d(cdim, cdim, kernel_size=1, bias=bias)

        self.project_conf = nn.Conv2d(cdim, 1, kernel_size=1, bias=bias)

    def forward(self, cs, ds):
        b, v, _, h, w = cs.shape

        cs = cs.view(b * v, -1, h, w)
        ds = ds.view(b * v, -1, h, w)

        cqkv = self.cqkv_dwconv(self.cqkv(cs))
        cqkv = cqkv.chunk(3, dim=1)

        dqk = self.dqk(ds)
        dqk = dqk.chunk(2, dim=1)

        cq, ck, cv = map(lambda t: rearrange(t, '(b v) (head d) h w -> b (h w) head v d', v=v, head=self.heads), cqkv)
        dq, dk = map(lambda t: rearrange(t, '(b v) (head d) h w -> b (h w) head v d', v=v, head=self.heads), dqk)
        
        cq = F.normalize(cq, dim=-1)
        ck = F.normalize(ck, dim=-1)
        dq = F.normalize(dq, dim=-1)
        dk = F.normalize(dk, dim=-1)

        attn = (cq @ ck.transpose(-2, -1)) * self.temperature
        attn_d = (dq @ dk.transpose(-2, -1))
        attn = (attn * attn_d).softmax(dim=-1)

        feats = (attn @ cv)
        feats = rearrange(feats, 'b (h w) head v d -> (b v) (head d) h w', h=h, w=w, head=self.heads)

        out = self.project_out(feats).view(b, v, -1, h, w)
        conf = torch.softmax(
            self.project_conf(feats).view(b, v, -1, h, w),
            dim=1
        )

        return torch.sum(out * conf, dim=1)