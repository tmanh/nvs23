...
class MLP(nn.Module):
    def __init__(self, dim, dim_out, num_layers, hidden_size=256):
        ...
        for ind, (layer_dim_in, layer_dim_out) in enumerate(zip(dims[:-1], dims[1:])):
            ...
            layers.extend([nn.Linear(layer_dim_in, layer_dim_out), nn.Identity() if is_last else nn.GELU()])
...
class Dino(nn.Module):
    ...
    def forward(self, x, return_embedding=False, return_projection=True, student_temp=None, teacher_temp=None):
        ...
        return (loss_fn_(teacher_proj_one, student_proj_two) + loss_fn_(teacher_proj_two, student_proj_one)) / 2
