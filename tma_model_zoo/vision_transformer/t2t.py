...
class T2TViT(nn.Module):
    def __init__(self, *, image_size, num_classes, dim, depth = None, heads = None, mlp_dim = None, pool = 'cls', channels = 3, dim_head = 64, dropout = 0., emb_dropout = 0., transformer = None, t2t_layers = ((7, 4), (3, 2), (3, 2))):
        ...
        for i, (kernel_size, stride) in enumerate(t2t_layers):
            ...
            layers.extend([nn.Identity() if is_first else RearrangeImage(), nn.Unfold(kernel_size=kernel_size, stride=stride, padding=stride // 2), Rearrange('b c n -> b n c'), nn.Identity() if is_last else Transformer(dim=layer_dim, heads=1, depth=1, dim_head=layer_dim, mlp_dim=layer_dim, dropout=dropout)])
